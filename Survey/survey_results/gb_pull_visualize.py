...
colFile = '../assets/gallbladder/lvl1/gb_level1.obj'
visFile = '../assets/gallbladder/lvl1/gb_level1.obj'
pngFile = '../assets/gallbladder/lvl1/frame_0242_ss.png'
...
fileName = ''
#camera var
cameraPosMax = [-0.117492, 0.056072, 0.0350349]
cameraOrInit = [0.495442,-0.552076,-0.478521,0.469858]
cameraLookInit = [2.84211, -0.131066, 0.334592]
camPosHist = [[],[]]
camOrHist = [[],[]]
camLookHist = [[],[]]
step = 0 
...
taskPosLock = [
    ...
    [-0.105746,0.120257,0.0298377], #at around 8-9 ish minutes in
    [-0.1274,0.02847,0.04127]

]
taskOrLock = [
    [0.581215,-0.376173,-0.354562,0.628465],
    [0.3927,-0.643,-0.5865,0.2972],
    [0.5592,-0.4562,-0.435,0.5385]
]
taskLookLock = [
    [2.52167,1.33669,-0.0701377],
    [0.06944,0.01201,0.05799],
    [0.07587,0.07114,0.04994]
]
...
def visualRefresh(self):
    ...
    root.Gallbladder.Visual.addObject('MeshOBJLoader', name='Surface', filename=visFile,rotation='0 0 0')
    ...
    root.Fat.addObject('OglModel',name='fatVisual',src='@fatSurface',texturename='../assets/gallbladder/lvl1/fat_level1_Material.001.png')
    root.Background.removeObject(root.Background.getObject('BgVisual'))
    root.Background.addObject('OglModel',name='BgVisual',src='@BgPlane',texturename='../assets/gallbladder/lvl1/plane_2_Material.002.png')
    # Sofa.Simulation.init(root.Liver)

def activateTask(self, taskNum):
    root = self.getContext()
    global fileName
    indexLs = [814, 596, 599] #highlighted index corresponding to task number
    os.makedirs(f'survey_results/{testerID.value.decode()}', exist_ok=True)
    fileName = f'survey_results/{testerID.value.decode()}/{testerID.value.decode()}_lvl1_task{taskNum+1}.txt'
...
class Controller(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if any(taskState):
            ...
            if camRad <= camRadMax+0.02 and currPos[1] < 0.15 and currPos[1] > -0.02 and currPos[0] < -0.03 and currPos[2] < 0.15 and currPos[2] > 0 and currPos[0] > -0.14:  #must satisfy to save
                camPosHist.pop(0)
                camOrHist.pop(0)
                camLookHist.pop(0)
                # print('updating')
...
def createScene(root):
    ...
    root.addObject('AttachBodyButtonSetting',stiffness="100")
    ...
    root.addObject('VisualStyle', name='dispStyle',displayFlags="hideVisualModels showBehavior")
    ...
    gallbladder.addObject('TetrahedronFEMForceField',name='FEM',youngModulus='50000',poissonRatio='0.4',method='large',computeVonMisesStress='1',showVonMisesStressPerNodeColorMap='1')
    ...
    collision.addObject('MeshOBJLoader',name='collisionLoader', filename=colFile, rotation='0 0 0')
    ...
    gallbladder.addObject('SphereROI',name='retracted_ind',centers=[-0.007599935, 0.060764447, 0.049598172], radii=0.005,position='@dofs.position')
    indForceROI = gallbladder.retracted_ind.indices.value
    pull_force = 4.62
    pull_vector = np.array([-0.82,0.58,0])
    ...
    visu.addObject('MeshOBJLoader', name='Surface', filename=visFile,rotation='0 0 0')
...
#extra stuff 
    ...
    proxy.addObject('MechanicalObject',name='proxyObj',position=[-0.007599935, 0.060764447, 0.049598172])    
    ...
    fat.addObject('MeshOBJLoader',name='fatSurface',filename="../assets/gallbladder/lvl1/fat_level1.obj")
    fat.addObject('OglModel',name='fatVisual',src='@fatSurface',texturename='../assets/gallbladder/lvl1/fat_level1_Material.001.png')
 

    #plane stuff
    background = root.addChild('Background')
    background.addObject('MeshOBJLoader', name="BgPlane", filename='../assets/gallbladder/lvl1/plane_2.obj')
    background.addObject('OglModel',name='BgVisual',src="@BgPlane",texturename='../assets/gallbladder/lvl1/plane_2_Material.002.png')
    print('hiiii',points[500], points[600])
def main():
