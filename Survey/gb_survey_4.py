...
mshFile = '../assets/gallbladder/lvl4/gb_4m.msh'
colFile = '../assets/gallbladder/lvl4/level4.obj'
visFile = '../assets/gallbladder/lvl4/level4.obj'
pngFile = '../assets/gallbladder/lvl4/frame_0074_lb.png'
...
fileName = ''
#camera var
cameraPosMax = [-0.125, 0.00382607, 0.0516089]
...
def visualRefresh(self):
    ...
    root.Background.addObject('OglModel',name='BgVisual',src='@BgPlane',texturename='../assets/gallbladder/lvl4/level4_plane_updated_Material.002.png')
    # Sofa.Simulation.init(root.Liver)
[541, 87, 354, 234]
def activateTask(self, taskNum):
    root = self.getContext()
    global fileName
    indexLs = [812, 1386,1125] #highlighted index corresponding to task number
    os.makedirs(f'survey_results/{testerID.value.decode()}', exist_ok=True)
    fileName = f'survey_results/{testerID.value.decode()}/{testerID.value.decode()}_lvl4_task{taskNum+1}.txt'
...
class Controller(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if any(taskState):
            ...
            if camRad <= camRadMax+0.02 and currPos[0] > -0.13 and currPos[1] < 0.4 and currPos[1] > -0.1 and currPos[0] < -0.03:
                camPosHist.pop(0)
                camOrHist.pop(0)
                camLookHist.pop(0)
                # print('updating')
...
def createScene(root):
    ...
    print(points[812])#[812, 1386,1125]
    print(points[1386])
    print(points[1125])
...
#extra stuff 
    ...
    background.addObject('MeshOBJLoader', name="BgPlane", filename='../assets/gallbladder/lvl4/level4_plane_updated.obj')
    background.addObject('OglModel',name='BgVisual',src="@BgPlane",texturename='../assets/gallbladder/lvl4/Level4_plane_updated_Material.002.png')
def main():
