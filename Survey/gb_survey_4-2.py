...
mshFile = '../assets/gallbladder/lvl4-2/gb_level4_2.msh'
colFile = '../assets/gallbladder/lvl4-2/level4_2_texture.obj'
visFile = '../assets/gallbladder/lvl4-2/level4_2_texture.obj'
pngFile = '../assets/gallbladder/lvl4-2/frame_0054.png'
...
fileName = ''
#camera var
cameraPosMax = [-0.1944, -0.008258, 0.05474]
cameraOrInit = [0.4994,-0.4830,-0.4960,0.5209]
cameraLookInit = [0.001675, -0.0002, 0.04792]
camPosHist = [[],[]]
camOrHist = [[],[]]
camLookHist = [[],[]]
step = 0 
...
taskPosLock = [
    ...
    [-0.132412,0.05936,0.05227], #at around 8-9 ish minutes in
    [-0.1552,-0.01846,0.0483]
]
taskOrLock = [
    [0.581215,-0.376173,-0.354562,0.628465],
    [-0.4252,0.5481,0.5639,-0.448],
    [-0.5538,0.4362,0.4108,-0.5782]
]
taskLookLock = [
    [2.52167,1.33669,-0.0701377],
    [0.0693,0.010045,0.04448],
    [0.06994,0.04774,0.04685]
]
...
def visualRefresh(self):
    ...
    # root.Fat.addObject('OglModel',name='fatVisual',src='@fatSurface',texturename='../assets/gallbladder/lvl4/level4_fat_update_Material.001.png')
    root.Background.removeObject(root.Background.getObject('BgVisual'))
    root.Background.addObject('OglModel',name='BgVisual',src='@BgPlane',texturename='../assets/gallbladder/lvl4-2/level4_2_plane_updated_Material.002.png')
    # Sofa.Simulation.init(root.Liver)
[541, 87, 354, 234]
def activateTask(self, taskNum):
    root = self.getContext()
    global fileName
    indexLs = [337, 1133,503] #highlighted index corresponding to task number
    os.makedirs(f'survey_results/{testerID.value.decode()}', exist_ok=True)
    fileName = f'survey_results/{testerID.value.decode()}/{testerID.value.decode()}_lvl4-2_task{taskNum+1}.txt'
...
class Controller(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if any(taskState):
            ...
            if camRad <= camRadMax+0.02 and currPos[0] > -0.2 and currPos[1] < 0.2 and currPos[1] > -0.2 and currPos[0] < -0.03:
                camPosHist.pop(0)
                camOrHist.pop(0)
                camLookHist.pop(0)
                # print('updating')
...
def createScene(root):
    ...
    gallbladder.addObject('PlaneForceField',normal='-1 0 0', d="-0.025", stiffness='1000',damping='1',showPlane='0')
...
#extra stuff 
    ...
    # fat.addObject('MeshOBJLoader',name='fatSurface',filename="../assets/gallbladder/lvl4/level4_fat_update.obj")
    # fat.addObject('OglModel',name='fatVisual',src='@fatSurface',texturename='../assets/gallbladder/lvl4/level4_fat_update_Material.001.png')
 
    background = root.addChild('Background')
    background.addObject('MeshOBJLoader', name="BgPlane", filename='../assets/gallbladder/lvl4-2/level4_2_plane_updated.obj')
    background.addObject('OglModel',name='BgVisual',src="@BgPlane",texturename='../assets/gallbladder/lvl4-2/Level4_2_plane_updated_Material.002.png')
def main():
    ...
    xBar = xMaxT - (xMaxT - xMinT)/2.5
    zBar = zMaxT - (zMaxT - zMinT)/2
    print(xBar)
